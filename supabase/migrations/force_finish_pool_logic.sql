-- FORCE UPDATE FINISH POOL FUNCTION
-- Identical logic to previous, re-run to ensure latest version is active.
...
BEGIN
    ...
    IF v_winners_count > 0 THEN
        ...
        INSERT INTO public.user_notifications (user_id, message, created_at)
        SELECT user_id, '❌ O bolão "' || v_pool.title || '" encerrou. Venceu: ' || p_winning_option, now()
