-- 🚨 FIX DOUBLE PAYOUT & CORRECT FEE (10%) 🚨
-- This script fixes two major issues:
-- 1. Double Payouts: Drops hidden triggers that might be duplicating the balance update.
-- 2. Fee Calculation: Sets the house fee to a flat 10% as requested.

-- ==============================================================================
-- 1. DROP POTENTIAL CONFLICTING TRIGGERS (KILL GHOST TRIGGERS)
-- ==============================================================================
-- We are removing any trigger that might be auto-updating the balance when a transaction is inserted.
-- The RPC function below will handle the update EXPLICITLY.
...
DROP TRIGGER IF EXISTS update_user_balance ON public.transactions;
DROP TRIGGER IF EXISTS transaction_update_balance ON public.transactions;

-- We also drop the functions they might call, just in case (optional, but cleaner)
DROP FUNCTION IF EXISTS public.update_balance_from_transaction();
DROP FUNCTION IF EXISTS public.handle_transaction_created();


-- ==============================================================================
-- 2. UPDATE FINISH POOL LOGIC (10% FEE + SINGLE PAYOUT)
-- ==============================================================================
...
BEGIN
    ...
    IF v_winners_count > 0 THEN
        ...
        FOR v_bet IN SELECT * FROM public.bets WHERE pool_id = p_pool_id AND selected_option = p_winning_option LOOP
            -- A. EXPLICIT UPDATE (Single Source of Truth)
            ...
            -- B. CREATE TRANSACTION (For Record Keeping Only - Triggers Removed)
