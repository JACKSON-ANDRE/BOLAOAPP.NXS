...
-- FIX DEPOSIT FLOW COMPLETE (CORRECTED BALANCE TYPE)
-- The error "violates check constraint transactions_balance_type_check" happened because 'real' is not valid.
-- The inspection showed that 'balance' IS a valid value.
...
BEGIN
  ...
  INSERT INTO user_notifications (user_id, message, created_at, message_id)
  VALUES (
  ...
  IF p_action = 'approve' THEN
    ...
    VALUES (
      ...
      p_admin_id,
      'balance', -- <--- CORRECTED: Using 'balance' as seen in the constraint definition
      now()
    );
  END IF;
