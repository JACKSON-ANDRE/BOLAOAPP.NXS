-- CORREÇÃO DEFINITIVA: INCLUIR NOTIFICAÇÃO NO REEMBOLSO
-- O código antigo devolvia o dinheiro mas ESQUECIA de avisar o usuário.
-- Vamos substituir pela versão corrigida.
...
BEGIN
    ...
    FOR v_bet IN SELECT * FROM bets WHERE pool_id = p_pool_id LOOP
        ...
        -- A. Devolve o dinheiro (Saldo Livre)
        UPDATE profiles 
        SET balance = balance + v_bet.amount 
        WHERE id = v_bet.user_id;

        -- B. Registra a transação de Reembolso
        ...
        -- C. (O QUE FALTAVA) --> Manda a Notificação!
